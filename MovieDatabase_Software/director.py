...
class Dir_win:
    ...
    def add_data(self):
        ...
        elif not self.var_dir_phone.get().isdigit() or len(self.var_dir_phone.get())<7:
             messagebox.showerror("Error","Enter a valid phone number(The phno must atleast have 7 digits)",parent=self.root)
        else:
            try:
                connection = mysql.connector.connect(host = 'localhost',username ='root',password = '3BR19CS040',database = 'moviedb')
